...
import { Camera, CheckCircle2, ImagePlus, Loader2, RefreshCw, ScanLine, UploadCloud, WandSparkles } from 'lucide-react'
...
const scopeOptions = [
  ...
  { key: 'plants', label: '植物' },
  { key: 'fungi', label: '真菌昆虫' },
  { key: 'phenomena', label: '自然现象' },
  { key: 'behaviors', label: '动物行为' },
]
...
const displaySpeciesName = (object: PhotoObject) => {
  const localized = localTaxonName({
...
export default function PhotoIdentifyPage() {
  ...
  const [scopes, setScopes] = useState(scopeOptions.map((item) => item.key))
    ...
    return {
      ...
      objects: data.objects.map((object) => {
        ...
        return {
          ...
          common_name_zh: guide.common_name_zh || object.common_name_zh,
  ...
  const reidentify = async () => {
    ...
    try {
      const data = await api<PhotoIdentifyResult>(`/api/identify/jobs/${result.job_id}/reidentify`, {
        method: 'POST',
        body: JSON.stringify({ hint, address, scopes: scopes.join(',') }),
      })
      const enriched = await enrichResultWithGuides(data)
      storeResult(enriched)
    } catch (reason) {
  ...
  return (
    <div className="page-stack photo-page">
      <section className="page-intro">
        <div><span className="eyebrow">CAMERA · IDENTIFY · LEARN</span><h2>生态识别</h2></div>
        ...
        <aside className="photo-control panel">
          ...
          <div className="identify-scope">
            {scopeOptions.map((item) => (
              <button
                type="button"
                className={scopes.includes(item.key) ? 'selected' : ''}
                key={item.key}
                onClick={() => toggleScope(item.key)}
              >
                <CheckCircle2 />{item.label}
              </button>
            ))}
          </div>
        ...
        <>
          ...
          {result.objects.length === 0 ? (
            <section className="panel empty-state">
              没有识别到符合当前类别选择的目标。可以调整多选类别、补充场景提示，或换一张主体更清晰的图片。
            </section>
          ) : (
            <section className="identified-grid">
              ...
              <button key={object.id} className="identified-card" onClick={() => setSelected(object)}>
                ...
                <div><span>{categoryLabels[object.category] || categoryNameZh(object.category)}</span><h3>{displaySpeciesName(object)}</h3><p>{object.explanation || '点击查看识别解释和中文科普。'}</p></div>
