  ...
  const filtered = useMemo(() => records.filter((item) => {
    const type = item.category === 'phenomenon' || item.category === 'fire' || item.category === 'smoke' || item.category === 'weather'
      ...
      : item.category === 'behavior'
        ? 'behavior'
        : 'species'
    return (!filter || type === filter) && `${item.title}${item.scientific_name}${item.category}`.toLowerCase().includes(query.toLowerCase())
          ...
          {filtered.map((record) => (
            <article key={`${record.species_id}-${record.scientific_name}-${record.title}`} className="discovery-entry type-species">
              ...
              <div className="discovery-card">
                <div className="discovery-image">{record.latest_image_url ? <img src={mediaUrl(record.latest_image_url)} alt={record.title} /> : <div><ImageIcon /></div>}<span>{record.category}</span></div>
                <div className="discovery-content"><div className="discovery-title"><div><h3>{record.title}</h3><em>{record.scientific_name || '学名待确认'}</em></div><strong><Repeat2 /> {record.count}</strong></div><p>首次：{new Date(record.first_discovered_at).toLocaleString('zh-CN')} · 最近：{new Date(record.last_discovered_at).toLocaleString('zh-CN')}</p><div className="discovery-meta"><span><Sparkles />累计 {record.count} 次观察</span><span>最新记录 #{record.latest_record_id}</span></div></div>
                <button className="share-record-btn" onClick={() => { setShareId(record.latest_record_id); setShareText(`我观察到${record.title}，累计记录 ${record.count} 次。`) }}><Share2 />分享</button>
              </div>
