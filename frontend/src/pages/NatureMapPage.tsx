...
export default function NatureMapPage() {
        ...
        chart.setOption({
          ...
          tooltip: {
            ...
            formatter: (params: any) => {
              ...
              return `<strong>${item.title}</strong><br/>${formatPlace(item)}<br/>置信度 ${(item.confidence * 100).toFixed(1)}%${item.is_first ? '<br/>★ 首次发现' : ''}`
            },
          },
          geo: {
            map: 'wildlens-china',
  ...
  return <div className="page-stack nature-map-page">
    ...
    {selected && <section className="panel map-selection-card">
      ...
      <div><span className="eyebrow">OBSERVATION #{selected.id}</span><h3>{selected.title}</h3><p>{formatPlace(selected)} · {new Date(selected.observed_at).toLocaleString('zh-CN')}</p><div className="selection-tags">{selected.is_first && <span>★ 首次发现</span>}{selected.behavior && <span>行为：{selected.behavior}</span>}{selected.phenomenon && <span>现象：{selected.phenomenon}</span>}<span>位置权限：{selected.privacy_level}</span></div></div>
    </section>}
