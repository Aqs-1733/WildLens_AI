  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setProfile({
        ...
        frequent_locations: (data.frequent_locations || []).join('，'),
      })
    } catch (err) {
    ...
    <div className="page-stack profile-page">
      ...
      <section className="panel profile-settings-panel profile-only-panel">
        ...
        <div className="profile-settings-grid">
          <div className="avatar-preview">{profile.avatar_url ? <img src={profile.avatar_url} alt="头像预览"/> : <span>{avatarText}</span>}</div>
          ...
          <label><MapPin size={16}/>所在或常去位置<input value={profile.home_location} onChange={(event) => setProfile((current) => ({ ...current, home_location: event.target.value }))} placeholder="例如 北京奥森、天津水上公园"/></label>
